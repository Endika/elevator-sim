...
describe('common random numbers — the guarantee the whole method rests on', () => {
  it('produces an identical stream for the same seed', () => {
    const a = generateStream(residential, UP_PEAK, 1);
    const b = generateStream(residential, UP_PEAK, 1);
    expect(a).toEqual(b);
  });

  it('produces different demand for different seeds', () => {
    const a = generateStream(residential, UP_PEAK, 1);
    const b = generateStream(residential, UP_PEAK, 2);
    expect(a.passengers).not.toEqual(b.passengers);
  });

  it('cannot be mutated by whoever is being judged on it', () => {
    const stream = generateStream(residential, UP_PEAK, 1);
...
describe('the shape of a stream', () => {
  const stream = generateStream(residential, UP_PEAK, 99);
  ...
  it('carries the seed and the scenario, so a result can be traced back', () => {
    ...
    expect(stream.building).toBe(residential.name);
  });
...
describe('down-peak', () => {
  it('ends every journey at an entrance', () => {
    const stream = generateStream(residential, { ...UP_PEAK, pattern: 'down-peak' }, 4);
...
describe('demand adds up to what was asked for', () => {
  it('averages the expected number of passengers across 30 seeds', () => {
    const counts = Array.from(
      { length: 30 },
      (_, seed) => generateStream(residential, UP_PEAK, seed + 1).passengers.length,
    );
    const mean = counts.reduce((a, b) => a + b, 0) / counts.length;
    const expected = expectedArrivals(UP_PEAK, residential.totalPopulation);
  ...
  it('keeps the same expected demand when arrivals are bursty', () => {
    ...
    const bursty = Array.from(
      { length: 30 },
      (_, seed) =>
        generateStream(residential, { ...UP_PEAK, burstiness: 3 }, seed + 1).passengers.length,
    );
  ...
  it('clumps arrivals when bursty and does not when smooth', () => {
    const smooth = generateStream(residential, UP_PEAK, 8);
    const bursty = generateStream(residential, { ...UP_PEAK, burstiness: 3 }, 8);
  ...
  it('gives a group a shared origin', () => {
    const bursty = generateStream(residential, { ...UP_PEAK, burstiness: 4 }, 8);
...
describe('refusing to generate nonsense', () => {
  it('rejects impossible demand instead of producing an empty morning', () => {
    expect(() => generateStream(residential, { ...UP_PEAK, demandPercentPer5Min: 0 }, 1)).toThrow(
      /greater than zero/,
    );
  });

  it('rejects a building where the only populated floor is the only way in', () => {
    const oneFloor = Building.of({
      ...RESIDENTIAL_LOW,
      floors: [
